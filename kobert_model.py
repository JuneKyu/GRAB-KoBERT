...
class BERTDataset(Dataset):
    def __init__(self, dataset, sent_idx, label_idx, bert_tokenizer, max_len,
        ...
        for i, data in enumerate(dataset.values):
            if (i + 1) % 1000 == 0:
                print("tokenizing " + str(i + 1) + " out of " + str(len(dataset)))
...
def kobert(MODEL_NAME, train_data, test_data, MAX_LEN = 32):
    ...
    model = BERTClassifier(bertmodel, dr_rate=0.5).to(device) # TODO : disable dropout
    ...
    for e in range(num_epochs):
        ...
        for batch_id, (token_ids, valid_length, segment_ids, label) in enumerate(train_dataloader):
            ...
            if (batch_id+1) % log_interval == 0:
                print("batch {:>5,} loss {:>5,} train acc {:>5,}".format(batch_id+1, loss.data.cpu().numpy(), train_acc / (batch_id+1)))
                log.info("batch {:>5,} loss {:>5,} train acc {:>5,}".format(batch_id+1, loss.data.cpu().numpy(), train_acc / (batch_id+1)))        
        print("epoch {:>5,} avg train acc {:>5,}".format(e+1, train_acc / (batch_id+1)))
        log.info("epoch {:>5,} avg train acc {:>5,}".format(e+1, train_acc / (batch_id+1)))
        ...
        print("epoch {:>5,} val acc {:>5,}".format(e+1, val_acc / (batch_id+1)))
        log.info("epoch {:>5,} val acc {:>5,}".format(e+1, val_acc / (batch_id+1)))
